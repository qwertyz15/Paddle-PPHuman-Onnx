...
def main():
    # Get model and video_url from the configuration
    ...
    # Convert video_url to int if it's a digit
    if video_url.isdigit():
        video_url = int(video_url)

    cuda = config.getboolean('Default', 'cuda')

    providers = ['CUDAExecutionProvider', 'CPUExecutionProvider'] if cuda else ['CPUExecutionProvider']
    predictor = InferenceSession(model, providers=providers)
    ...
    while True:
        ...
        # Call functions for prediction and drawing bounding boxes
